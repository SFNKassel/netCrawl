...
#define MAXPACKET (65536 - 60 - ICMP_MINLEN) /* max packet size */
    ...
    int ping(string target) {
        ...
        // try to convert as dotted decimal address, else if that fails assume
        // it's a hostname
        ...
        */
        ...
        if(i < 0 || i != cc) {
            ...
            // cout << "wrote " << hostname << " " << cc << " chars, ret= " << i
                 // << endl;
        }
        ...
        while(cont) {
            ...
            if(retval == -1) {
                ...
                if((ret = recvfrom(s, (char *)packet, packlen, 0,
                ...
                if(ret < (hlen + ICMP_MINLEN)) {
                    cerr << "packet too short (" << ret << " bytes) from "
                         << hostname << endl;
                    ;
                    return -1;
                }
                ...
                end_t = 1000000 * (end.tv_sec - start.tv_sec) +
